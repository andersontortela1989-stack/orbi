...
import {gardenView,copySchoolArt} from '../src/components/school/school-living-model.js';
import {createHorta,HORTA_BANDS} from '../src/adventures/horta.js';
...
test('publicação é explícita, preserva economia e não acrescenta campos no save',()=>{
 useGame.getState().resetar();const board=Array(16).fill(null);board[0]={shape:'triangle',color:'grass'};
 const s=useGame.getState();assert.equal(s.publicarArteEscola(board),true);board[0].color='sun';
 assert.equal(useGame.getState().arteEscola[0].color,'grass');assert.equal(s.publicarArteEscola([]),false);
 assert.equal(useGame.getState().moedas,0);const saved=useGame.persist.getOptions().partialize(useGame.getState());assert.equal(Object.keys(saved).length,11);assert.equal('arteEscola' in saved,false);
 s.resetar();assert.equal(useGame.getState().arteEscola,null);
});
