/* Órbi — astronauta sticker · gerador de SVG reutilizável.
   ----------------------------------------------------------------
   Port ES-module do gerador do design system
   (orbi-brand/.../assets/orbi.js). Mesmo desenho, mesmas poses;
   só troquei o wrapper IIFE/`window.OrbiChar` por exports nomeados
   pra consumir no React (StartScreen na Fase 2; HUD/momentos na 3).

   Clima Mundo Adesivo: contorno navy grosso, cores de brincar, formas
   macias. Técnica de membro: traço navy grosso + traço branco por cima
   = cápsula com contorno.

   API:
     orbiSvg(pose, opts)   pose: 'acenando'|'comemorando'|'curioso'|'parado'
                           opts.bust:true → só cabeça+tronco (p/ ícone)
     orbiFaceSvg(mood)     mood: 'feliz'|'uau'|'curioso'|'calmo'
     orbiInner(pose)       string interna (sem <svg>)
     ORBI_PALETTE
   ---------------------------------------------------------------- */
...
function limb(d) {
  return (
    '<path d="' + d + '" fill="none" stroke="' + NAVY + '" stroke-width="30" stroke-linecap="round"/>' +
    '<path d="' + d + '" fill="none" stroke="' + WHITE + '" stroke-width="16" stroke-linecap="round"/>'
  );
}
function hand(cx, cy) {
...
function star(cx, cy, r, fill) {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    p +=
      (i === 0 ? 'M' : 'L') +
      (cx + Math.cos(ang) * rad).toFixed(1) +
      ' ' +
      (cy - Math.sin(ang) * rad).toFixed(1) +
      ' ';
  }
...
function face(mood) {
  let s =
    '<circle cx="86" cy="112" r="6" fill="' + CORALSOFT + '"/><circle cx="134" cy="112" r="6" fill="' + CORALSOFT + '"/>';
  if (mood === 'curioso') {
...
function head(mood) {
  return (
    '<circle cx="110" cy="92" r="56" fill="' + WHITE + '" stroke="' + NAVY + '" stroke-width="8"/>' +
    '<path d="M80 64 Q92 52 108 52" fill="none" stroke="' + SUN + '" stroke-width="7" stroke-linecap="round" opacity="0.9"/>' +
    '<rect x="68" y="74" width="84" height="58" rx="29" fill="' + BLUESOFT + '" stroke="' + NAVY + '" stroke-width="7"/>' +
    face(mood)
  );
}
...
export function orbiInner(pose) {
  return (
    '' +
    '<line x1="110" y1="40" x2="110" y2="26" stroke="' + NAVY + '" stroke-width="6" stroke-linecap="round"/>' +
    '<circle cx="110" cy="18" r="8" fill="' + SUN + '" stroke="' + NAVY + '" stroke-width="4"/>' +
    limb('M94 214 L90 236') +
    limb('M126 214 L130 236') +
    '<ellipse cx="88" cy="240" rx="17" ry="11" fill="' + CORAL + '" stroke="' + NAVY + '" stroke-width="7"/>' +
    '<ellipse cx="132" cy="240" rx="17" ry="11" fill="' + CORAL + '" stroke="' + NAVY + '" stroke-width="7"/>' +
    arms(pose) +
    '<rect x="64" y="138" width="92" height="86" rx="36" fill="' + WHITE + '" stroke="' + NAVY + '" stroke-width="8"/>' +
...
export function orbiSvg(pose, opts) {
  opts = opts || {};
  const vb = opts.bust ? '40 0 140 150' : '0 0 220 250';
  return '<svg viewBox="' + vb + '" xmlns="http://www.w3.org/2000/svg">' + orbiInner(pose) + '</svg>';
}

// só o capacete + rosto, p/ tira de expressões e favicons
export function orbiFaceSvg(mood) {
  return '<svg viewBox="50 32 120 120" xmlns="http://www.w3.org/2000/svg">' + head(mood) + '</svg>';
}

export const ORBI_PALETTE = { NAVY, SUN, BLUE, CORAL, BLUESOFT };
